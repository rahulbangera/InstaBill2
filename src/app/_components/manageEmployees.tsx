...
const ManageEmployees = ({ shopId }: { shopId: string }) => {
  ...
  return (
    <div className="mx-auto flex h-screen max-w-4xl flex-col bg-gray-900 p-6 text-white">
      <h1 className="mb-6 text-center text-3xl font-bold text-blue-400">Manage Employees</h1>
      ...
      <div className="max-h-[400px] overflow-y-auto rounded-lg border border-gray-600 bg-gray-800 p-4">
        {employees.length > 0 ? (
          employees.map((emp) => (
            <div key={emp.id} className="mb-2 flex items-center justify-between rounded-lg bg-gray-700 p-3">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-1/4 rounded-lg bg-gray-700 p-5">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setShowDeleteModal(false)} className="rounded bg-gray-400 px-3 py-1 text-white">
                Cancel
              </button>
              <button onClick={() => removeEmployee()} className="rounded bg-red-500 px-3 py-1 text-white">
                Confirm
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-1/3 rounded-lg bg-gray-700 p-5">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setEmployeeModalOpen(false)} className="rounded bg-gray-400 px-3 py-1 text-white">
                Cancel
              </button>
